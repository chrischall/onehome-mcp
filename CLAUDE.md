...
- **`view` is a PARTIAL rollout — four tools, not twenty.** `onehome_get_by_address`, `onehome_compare_properties`, `onehome_graphql` and `onehome_get_user` declare `view: viewArg()` and answer through `viewResponse(view, data)` (`src/view.ts`). Every other tool still calls `minifiedResult` directly and takes no `view` at all — `properties.ts`, `search.ts`, `bulk-get.ts`, `saved.ts`, `saved-with-listings.ts`, `schools.ts`, `resolve-addresses.ts`, `photos.ts`, and the pure-computation and status tools. That is scope, not oversight, and it is written down here so the list can be read rather than guessed at; extending it is a follow-up.
- **Within that set, the rule is absolute: a tool that ADVERTISES `view` must honour it on EVERY exit.** Calling `minifiedResult` from such a handler makes the parameter a no-op — a schema promising a response shape the handler never delivers. `onehome_get_user` shipped that way with `view` honoured on neither of its two exits, and a tool with one wired branch and one unwired branch is the same bug wearing a disguise.
- **Destructure `view` off the args before building any upstream request.** It is a response-shape argument and OneHome has never heard of it. `onehome_get_by_address` passes its whole input object down to the address-query builder, which is exactly the shape in which two sibling repos started sending `view=compact` to a live API.
- **`stripMediaUrls` is only safe because it is SUBTRACTIVE** — it names what to remove, never what to keep, so a field nobody anticipated cannot be lost. Never apply it to a tool whose PRODUCT is the image: `onehome_get_property_photos` returns exactly those URLs, and stripping there does not shrink the response, it empties it. That tool takes no `view`.
- Tool annotations: every tool sets `title`, `readOnlyHint: true`, `idempotentHint: true`, and `openWorldHint`. The last is `true` for GraphQL-bound tools and `false` for `onehome_calculate_mortgage` / `onehome_calculate_affordability` (pure local computation). `onehome_graphql` keeps `readOnlyHint: true` honest by refusing any document with a `mutation` / `subscription` operation (`topLevelOperationKinds`); it still sets `idempotentHint: false` since it forwards arbitrary queries.
